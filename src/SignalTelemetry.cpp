...
void SignalTelemetry::begin(MQTTManager* mqtt, const String& macAddr) {
    ...
    if (signalRingBuffer == nullptr) {
        Serial.println("ERROR: Failed to create signal ring buffer");
        return;
    }
    ...
    if (batchQueue == nullptr) {
        Serial.println("ERROR: Failed to create batch queue");
        return;
    }
    ...
    if (result != pdPASS) {
        Serial.println("ERROR: Failed to create signal collect task");
        return;
    }
    ...
    if (result != pdPASS) {
        Serial.println("ERROR: Failed to create signal publish task");
        return;
    }
    
    Serial.println("SignalTelemetry initialized");
}
...
bool SignalTelemetry::configurePin(uint8_t pin, bool captureRaw, bool capturePulse, bool useRMT) {
    ...
    config.rawTopic = "raw/" + String(pin);
    config.pulseTopic = "pulse/" + String(pin);
    ...
    if (capturePulse && useRMT) {
        ...
        if (configureRMT(pin, channel)) {
            ...
            Serial.println("WARNING: RMT configuration failed for pin " + String(pin) + ", using ISR fallback");
            config.useRMT = false;
        }
    }
    ...
    Serial.println("Pin " + String(pin) + " configured for signal telemetry");
    Serial.println("  Raw: " + String(captureRaw ? "Yes" : "No"));
    Serial.println("  Pulse: " + String(capturePulse ? "Yes" : "No"));
    Serial.println("  RMT: " + String(config.useRMT ? "Yes" : "No"));
...
bool SignalTelemetry::removePin(uint8_t pin) {
    ...
    Serial.println("Pin " + String(pin) + " removed from signal telemetry");
    return true;
}
...
void SignalTelemetry::publishPulse(const PulsePacket* pulse) {
    ...
    String topic = "pulse/" + String(pulse->pinId);
...
void SignalTelemetry::publishDiagnostics() {
    ...
    Serial.println("  Dropped Raw: " + String(diag.droppedRaw));
    Serial.println("  Dropped Pulse: " + String(diag.droppedPulse));
    Serial.println("  Queue Depth: " + String(diag.queueDepth));
    Serial.println("  RMT Overflow: " + String(diag.rmtOverflow));
}
...
void SignalTelemetry::publishHeartbeat() {
    ...
    String payload = "{\"mac\":\"" + macAddress + "\",\"seq\":" + String(seq) + ",\"uptime\":" + String(millis()/1000) + "}";
...
void SignalTelemetry::onBoot() {
    ...
    Serial.println("SignalTelemetry boot sequence complete");
}
...
bool SignalTelemetry::configureRMT(uint8_t pin, rmt_channel_t channel) {
    ...
    if (err != ESP_OK) {
        Serial.println("ERROR: RMT config failed: " + String(err));
        return false;
    }
    
    err = rmt_driver_install(channel, 1000, 0);
    if (err != ESP_OK) {
        Serial.println("ERROR: RMT driver install failed: " + String(err));
        return false;
    }
    ...
    if (err != ESP_OK) {
        Serial.println("ERROR: RMT rx start failed: " + String(err));
