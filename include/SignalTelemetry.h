...
struct SignalPinConfig {
    ...
    String rawTopic;       // Topic for raw edge data
    String pulseTopic;     // Topic for pulse width data
};
...
private:
    ...
    bool configureRMT(uint8_t pin, rmt_channel_t channel);
...
public:
    ...
    void begin(MQTTManager* mqtt, const String& macAddr);
    void loop();
